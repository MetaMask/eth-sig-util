...
test('personalSign and recover', function (t) {
  ...
  const msgParams: sigUtil.MsgParams = { data: message }

  const signed = sigUtil.personalSign(privKey, msgParams)
  msgParams.sig = signed
  const recovered = sigUtil.recoverPersonalSignature(msgParams as sigUtil.SignedMsgParams)
...
test('personalSign and extractPublicKey', function (t) {
  ...
  const signed = sigUtil.personalSign(privKey, msgParams)
  msgParams.sig = signed
  const publicKey = sigUtil.extractPublicKey(msgParams as sigUtil.SignedMsgParams)
